...
string Contract::SetupContractData(const string *func, ...) {
	LOG("Contract::SetupContractData");
	string ret = "";
    ...
    for( int i = 0; i < paramCount; ++i ) {
        if (strncmp(params[i].c_str(), "uint", sizeof("uint")) == 0) {
            ...
            ret = ret + string(output);
        } else if (strncmp(params[i].c_str(), "uint256", sizeof("uint")) == 0) {
            string output = GenerateBytesForUint(va_arg(args, uint32_t));
            ret = ret + output;
        }
    }
    ...
    LOG("====================================");
#endif
	LOG("Contract::SetupContractData - Return");
...
string Contract::GenerateContractBytes(const string* func) {
    LOG("Contract::GenerateContractBytes");
	string in = "0x";
    char intmp[8];
    memset(intmp, 0, 8);
    int i = 0;
	...
	LOG("Contract::GenerateContractBytes - Return");
    return out;
}
